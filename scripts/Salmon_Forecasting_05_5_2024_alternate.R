...
port_moller_location = c(55.9886, -160.5769, "Port Moller")
district_df = read.csv("data/District_Coordinates.csv" )

district_map <- ne_countries(continent='North America', returnclass = 'sf') %>%  # grab country borders in Africa
  ggplot() + #coord_sf(xlim=c(-180, -150),ylim=c(51,60)) +
...
district_map <- ne_countries(continent='North America', returnclass = 'sf')

district_map %>%  # grab country borders in Africa
  ggplot() + geom_sf() +
  labs(title='Alaska')
...
ak <- read_sf("data/Alaska_Coastline/Alaska_Coastline.shp") #Alaska Albers 3338
ak_regions <- read_sf("data/sasap_regions.shp")
fishing_districts <- read_sf("data/BB_Sockeye_Districts_WGS1984/PVB_BB_Districts_2019_GCS_WGS84.shp")
#ak_regions <- read_sf("data/hydro.shp")
...
district_df_2 <-district_df %>% 
  st_as_sf(coords = c("long", "lat"), crs = 3467)
...
ggplot() + geom_sf(data=ak)  +
  ...
  theme_classic() +
  #geom_point(data = district_df, aes(x = long, y = lat), size = 1)+
  geom_sf(data = district_df_2, size = 2) + 
  coord_sf(xlim = c(-164,-156), ylim = c(56, 60)) 
...
# 
# ggplot() + geom_sf(data=usa) + geom_path(data=district_df,aes(x=long,y=lat,color=District)) + 
#   coord_sf(xlim = c(-164,-156), ylim = c(56, 60)) 


st_crs(ak)
st_crs(district_df_2)
...
library(sf)
#> Linking to GEOS 3.10.1, GDAL 3.3.3, PROJ 7.2.1; sf_use_s2() is TRUE
library(ggplot2)
library(rnaturalearth)
...
alaska <-
  ...
  st_as_sf(coords = c("lat", "long"), crs = 3467)#Alaska Albers


alaska_plot <- ggplot() +
  
  geom_sf(data = alaska) +
  geom_rect(aes(xmin = -165, xmax = -156, ymin = 56, ymax = 60),  fill = NA, colour = "black",)+
  #  geom_sf(data = fishing_districts, aes(fill = as.factor(DISTRICT_N)))+
  #labs(xlab = "Longitude", ylab = "Latitude") +
  ...
  coord_sf(xlim = c(-175, -130),
           ylim = c(50, 73)) 
# coord_sf(xlim = c(-164, -156),
#          ylim = c(56, 60)) 

alaska_plot

# 
# xlim = sf::st_bbox(alaska)
# ylim = sf::st_bbox(alaska)

alaska_plot_zoom <- ggplot() +
  geom_sf(data = alaska) +
  geom_sf(data = fishing_districts, aes(fill = as.factor(DISTRICT_N)))+
  #labs(xlab = "Longitude", ylab = "Latitude") +
  guides(fill=guide_legend(title="District")) +
  ...
  theme(legend.direction = "vertical", 
        ...
        legend.title = element_blank(),
        plot.margin = unit(c(0,0,1,40), "mm"))+
  # coord_sf(xlim = c(-175, -130),
  #          ylim = c(50, 73)) 
  coord_sf(xlim = c(-165, -156),
           ylim = c(56, 60))
