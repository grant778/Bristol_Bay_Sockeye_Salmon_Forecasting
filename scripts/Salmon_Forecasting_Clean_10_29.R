...
MAPE_in_sample = Mean_Percent_Absolute_Error(within_sample_predictions, observations=obs)


#inverse variance for the top 5 inseason models (by MAPE)

inverse_MAPE[[step]] = 1/MAPE_in_sample[best_models] 
model_weights[[step]] = inverse_MAPE[[step]] /sum(inverse_MAPE[[step]])
...
MAPE_from_ensemble_within_sample[[step]] = Mean_Percent_Absolute_Error(weighted_predictions[[step]], observations=obs[14:length(retro_years)])                            

length(seq(1993, (test_start_years[i]-1),1))

MAPE_From_preaseason[[step]] = Mean_Percent_Absolute_Error(data.frame(as.matrix(FRI_Preseason_Total_Forecast_1993_2023[1:(length(seq(1993, (test_start_years[i]-1),1)))], ncol = 1)), observations = obs[14:length(retro_years)])

inverse_MAPE_from_inseason_ensemble_and_preseason[[step]]= 1/c(MAPE_from_ensemble_within_sample[[step]], MAPE_From_preaseason[[step]])

#This calculates weights of weighted inseason models and preseason forecast

final_model_weights[[step]] = inverse_MAPE_from_inseason_ensemble_and_preseason[[step]]/sum(inverse_MAPE_from_inseason_ensemble_and_preseason[[step]])
...
for(j in 1:length(days_for_MAPE_Graph))
{
  ...
  for(i in 1:length(test_start_years))
  {
    ...
    MAPE_from_ensemble_within_sample[[step]] = Mean_Percent_Absolute_Error(weighted_predictions[[step]], observations=Total_Run_1980_2023[14:length(retro_years)])                            
    
    #length(seq(1993, (test_start_years[i]-1),1))
    
    MAPE_From_preaseason[[step]] = Mean_Percent_Absolute_Error(data.frame(as.matrix(FRI_Preseason_Total_Forecast_1993_2023[1:(length(seq(1993, (test_start_years[i]-1),1)))], ncol = 1)), observations = Total_Run_1980_2023[14:length(retro_years)])
